'''
Simple random pokerbot, written in Python.
'''
...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        action = random.choice(list(round_state.legal_actions()))
        if action is RaiseAction:
            action = action(random.randint(round_state.raise_bounds()[0], round_state.raise_bounds()[0]+5))
        else:
            action = action()
        return action
